...
int main()
{
    ...
    printf("[+] Trying to inject the new shared library into process with pid %d !\n", pid);
	if (injector_inject(injector, "pwnCheat.so", &handle) != 0) {
        printf("[-] INJECT ERROR: %s\n", injector_error());
		return -1;
    }
	printf("[+] Shared library successfully injected to pid %d !\n", pid);
	
    // printf("[+] Trying to unload previously loaded library !\n");
    
    // if (injector_uninject(injector, handle) != 0) {
    //     printf("[-] Could not unload previously shared loaded library! Goodluck !\n");
    // }
